...
import org.promefrut.simefrut.struts.catalogs.beans.CountryBean;
 ...
 *
 */
public class CountryAction extends DispatchBaseAction {
	...
	public void Init(HttpServletRequest request) throws Exception, Error {
		...
		title = super.bundle.getString("country.label.text");
		title = "" + title;
	}
	...
	public String load(ActionForm form, HttpServletRequest request,
		...
		//CountryForm countryForm = (CountryForm)form;
		CountryBean countryBean = new CountryBean(managerDB, bundle);
		
		resultJSON= countryBean.find();
	...
	public boolean ajaxInsert(ActionForm form, HttpServletRequest request,
		...
		countryForm.validateInsert(null, request, errors);
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			countryBean.insert(countryForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.insert", title, countryForm.getCtrISO3()));
		} else {
			success = false;
		}
	...
	public boolean ajaxUpdate(ActionForm form, HttpServletRequest request,
		...
		UserForm user;
		countryForm.validateUpdate(null, request, errors);

		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			countryBean.update(countryForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.update", title, countryForm.getCtrISO3()));
		} else {
			success = false;
		}
