...
import org.promefrut.simefrut.struts.catalogs.beans.ProvinceBean;
 ...
 *
 */
public class ProvinceAction extends DispatchBaseAction {
	...
	public void Init(HttpServletRequest request) throws Exception, Error {
		...
		title = super.bundle.getString("province.label.text");
		title = "" + title;
	}
	...
	public String load(ActionForm form, HttpServletRequest request,
		...
		ProvinceBean provinceBean = new ProvinceBean(managerDB, bundle);
		String ctrId = (String)request.getParameter("ctrId");
		String regId = (String)request.getParameter("regId");
		
		resultJSON= provinceBean.find(ctrId, regId);
	...
	public boolean ajaxInsert(ActionForm form, HttpServletRequest request,
		...
		provinceForm.validateInsert(null, request, errors);
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			provinceBean.insert(provinceForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.insert", title, provinceForm.getProvDesc()));
		} else {
			setFLAG_UPDATE(true);
		}
	...
	public boolean ajaxUpdate(ActionForm form, HttpServletRequest request,
		...
		boolean success = true;
		provinceForm.validateUpdate(null, request, errors);

		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			provinceBean.update(provinceForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.update", title, provinceForm.getProvDesc()));
		} else {
			success = false;
		}
