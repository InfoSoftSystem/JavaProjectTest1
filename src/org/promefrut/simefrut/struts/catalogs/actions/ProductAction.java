...
import org.promefrut.simefrut.struts.catalogs.beans.ProductBean;
 ...
 *
 */
public class ProductAction extends DispatchBaseAction {
	...
	public void Init(HttpServletRequest request) throws Exception, Error {
		...
		title = super.bundle.getString("product.label.text");
		title = "" + title;
	}
	...
	public String load(ActionForm form, HttpServletRequest request,
		...
		ProductBean productBean = new ProductBean(managerDB, bundle);
		
		resultJSON= productBean.find();
	...
	public boolean ajaxInsert(ActionForm form, HttpServletRequest request,
		...
		productForm.validateInsert(null, request, errors);
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			productBean.insert(productForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.insert", title, productForm.getProdDesc()));
		} else {
			setFLAG_UPDATE(true);
		}
	...
	public boolean ajaxUpdate(ActionForm form, HttpServletRequest request,
		...
		boolean success = true;
		productForm.validateUpdate(null, request, errors);

		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			productBean.update(productForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.update", title, productForm.getProdDesc()));
		} else {
			success = false;
		}
