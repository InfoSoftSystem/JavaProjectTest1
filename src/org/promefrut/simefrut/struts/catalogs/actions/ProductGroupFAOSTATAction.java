...
import org.promefrut.simefrut.struts.catalogs.beans.ProductGroupFAOSTATBean;
 ...
 *
 */
public class ProductGroupFAOSTATAction extends DispatchBaseAction {
	...
	public void Init(HttpServletRequest request) throws Exception, Error {
		...
		title = super.bundle.getString("productGroupFAOSTAT.label.text");
		title = "" + title;
	}
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		ProductGroupFAOSTATForm productGroupFAOSTATForm = (ProductGroupFAOSTATForm)form;
		
		if(!isFLAG_UPDATE()) {
			productGroupFAOSTATForm.reset(mapping, request);
		} else {
	...
	public String load(ActionForm form, HttpServletRequest request,
		...
		ProductGroupFAOSTATBean productGroupFAOSTATBean = new ProductGroupFAOSTATBean(managerDB, bundle);
		
		resultJSON= productGroupFAOSTATBean.find();
	...
	public boolean ajaxInsert(ActionForm form, HttpServletRequest request,
		...
		productGroupFAOSTATForm.validateInsert(null, request, errors);
		String anotherCountry = productGroupFAOSTATBean.existsData(productGroupFAOSTATForm);
		
		if(errors.isEmpty()) {
			if(!StringUtils.isBlank(anotherCountry)){
				errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("productGroupFAOSTAT.grpDesc.exists", anotherCountry));
			}
		}
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			productGroupFAOSTATBean.updateDW(productGroupFAOSTATForm, user.getUsername(),"A");
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.insert", title, productGroupFAOSTATForm.getGrpDesc()));
		} else {
			success = false;
		}
	...
	public boolean ajaxUpdate(ActionForm form, HttpServletRequest request,
		...
		boolean success = true;
		productGroupFAOSTATForm.validateUpdate(null, request, errors);
		
		String anotherCountry = productGroupFAOSTATBean.existsData(productGroupFAOSTATForm);
		
		if(errors.isEmpty()) {
			if(!StringUtils.isBlank(anotherCountry)){
				errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("productGroupFAOSTAT.grpId.exists", anotherCountry));
			}
		}
		...
		if(errors.isEmpty()) {
			...
			productGroupFAOSTATBean.updateDW(productGroupFAOSTATForm, user.getUsername(), "A");
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.update", title, productGroupFAOSTATForm.getGrpDesc()));
		} else {
			success = false;
		}
		saveErrors(request, errors);
	...
	public boolean ajaxDelete(ActionForm form, HttpServletRequest request,
		...
		boolean success = true;
		productGroupFAOSTATForm.validateDelete(null, request, errors);
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			productGroupFAOSTATBean.updateDW(productGroupFAOSTATForm, user.getUsername(),"I");
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.delete", title, productGroupFAOSTATForm.getGrpDesc()));
		} else {
			success = false;
		}
		saveErrors(request, errors);
