...
import org.promefrut.simefrut.struts.catalogs.beans.ProductTypeBean;
 ...
 *
 */
public class ProductTypeAction extends DispatchBaseAction {
	...
	public void Init(HttpServletRequest request) throws Exception, Error {
		...
		title = super.bundle.getString("productType.label.text");
		title = "" + title;
	}
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		if(!isFLAG_UPDATE()) {
			productTypeForm.reset(mapping, request);
		} else {
	...
	public String load(ActionForm form, HttpServletRequest request,
		...
		ProductTypeBean productTypeBean = new ProductTypeBean(managerDB, bundle);
		String prodId = (String)request.getParameter("prodId");
		
		resultJSON= productTypeBean.find(prodId);
	...
	public boolean ajaxInsert(ActionForm form, HttpServletRequest request,
		...
		productTypeForm.validateInsert(null, request, errors);
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			productTypeBean.insert(productTypeForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.insert", title, productTypeForm.getPtypeDesc()));
		} else {
			setFLAG_UPDATE(true);
		}
	...
	public boolean ajaxUpdate(ActionForm form, HttpServletRequest request,
		...
		boolean success = true;
		productTypeForm.validateUpdate(null, request, errors);

		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			productTypeBean.update(productTypeForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.update", title, productTypeForm.getPtypeDesc()));
		} else {
			success = false;
		}
