...
import org.promefrut.simefrut.struts.catalogs.beans.RegionBean;
 ...
 *
 */
public class RegionAction extends DispatchBaseAction {
	...
	public void Init(HttpServletRequest request) throws Exception, Error {
		...
		title = super.bundle.getString("region.label.text");
		title = "" + title;
	}
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		if(!isFLAG_UPDATE()) {
			regionForm.reset(mapping, request);
		} else {
	...
	public String load(ActionForm form, HttpServletRequest request,
		...
		RegionBean regionBean = new RegionBean(managerDB, bundle);
		String ctrId = (String)request.getParameter("ctrId");
		
		resultJSON= regionBean.find(ctrId);
	...
	public boolean ajaxInsert(ActionForm form, HttpServletRequest request,
		...
		regionForm.validateInsert(null, request, errors);
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			regionBean.insert(regionForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.insert", title, regionForm.getRegDesc()));
		} else {
			setFLAG_UPDATE(true);
		}
	...
	public boolean ajaxUpdate(ActionForm form, HttpServletRequest request,
		...
		boolean success = true;
		regionForm.validateUpdate(null, request, errors);

		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			regionBean.update(regionForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.update", title, regionForm.getRegDesc()));
		} else {
			success = false;
		}
