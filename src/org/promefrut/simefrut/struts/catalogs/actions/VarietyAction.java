...
import org.promefrut.simefrut.struts.catalogs.beans.VarietyBean;
 ...
 *
 */
public class VarietyAction extends DispatchBaseAction {
	...
	public void Init(HttpServletRequest request) throws Exception, Error {
		...
		title = super.bundle.getString("variety.label.text");
		title = "" + title;
	}
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		VarietyForm varietyForm = (VarietyForm)form;
		
		if(!isFLAG_UPDATE()) {
			varietyForm.reset(mapping, request);
		} else {
	...
	public String load(ActionForm form, HttpServletRequest request,
		...
		VarietyBean varietyBean = new VarietyBean(managerDB, bundle);
		String prodId = (String)request.getParameter("prodId");
		
		resultJSON= varietyBean.find(prodId);
	...
	public boolean ajaxInsert(ActionForm form, HttpServletRequest request,
		...
		varietyForm.validateInsert(null, request, errors);
		
		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			varietyBean.insert(varietyForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.insert", title, varietyForm.getVarDesc()));
		} else {
			setFLAG_UPDATE(true);
		}
	...
	public boolean ajaxUpdate(ActionForm form, HttpServletRequest request,
		...
		boolean success = true;
		varietyForm.validateUpdate(null, request, errors);

		if(errors.isEmpty()) {
			user = (UserForm)session.getAttribute("user");
			varietyBean.update(varietyForm, user.getUsername());
			managerDB.commit();
			errors.add(BaseForm.GLOBAL_MESSAGES, new ActionError("mensaje.exito.update", title, varietyForm.getVarDesc()));
		} else {
			success = false;
		}
