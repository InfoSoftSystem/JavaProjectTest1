...
import org.promefrut.simefrut.struts.reports.beans.QueryImportExportBean;
 ...
 *
 */
public class QueryImportExportAction extends DispatchBaseAction {
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		QueryImportExportBean queryImportExportBean = new QueryImportExportBean(managerDB, bundle);
		
		request.setAttribute("yearsCollection", queryImportExportBean.getYearsCollection());
	...
	public ActionForward loadCountries(ActionMapping mapping, ActionForm form, HttpServletRequest request, 
		...
		try {
		    ...
		    if(StringUtils.isEmpty(initialDate)){
		    	resultJSON += getAjaxTemplate(false, bundle.getString("queryImportExport.initialDate.required"));
		    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryImportExport.initialDate.required"));
		    }
		    
		    if(StringUtils.isEmpty(finalDate)){
		    	resultJSON += getAjaxTemplate(false, bundle.getString("queryImportExport.finalDate.required"));
		    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryImportExport.finalDate.required"));
		    }
		    
		    if(StringUtils.isEmpty(resultJSON)){
		    	resultJSON= queryImportExportBean.getCountriesCollection(initialDate, finalDate);
		    }else{
	...
	public ActionForward loadProducts(ActionMapping mapping, ActionForm form, HttpServletRequest request, 
		...
		try {
		    ...
		    if(StringUtils.isEmpty(countries)){
		    	resultJSON += getAjaxTemplate(false, bundle.getString("queryImportExport.countries.selected.required"));
		    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryImportExport.countries.selected.required"));
		    }
		    
		    if(StringUtils.isEmpty(resultJSON)){
		    	resultJSON= queryImportExportBean.getProductsCollection(initialDate, finalDate, countries);
		    }else{
		    	saveErrors(request, errors);
