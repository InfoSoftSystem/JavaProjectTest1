...
import org.promefrut.simefrut.struts.reports.beans.QueryCompetitivenessMatrixBean;
 ...
 *
 */
public class QueryCompetitivenessMatrixAction extends DispatchBaseAction {
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		QueryCompetitivenessMatrixBean queryCompetitivenessMatrixBean = new QueryCompetitivenessMatrixBean(managerDB, bundle);
		
		request.setAttribute("yearsCollection", queryCompetitivenessMatrixBean.getYearsCollection());
	...
	public String loadOffererCountries(HttpServletRequest request,SessionManager managerDB) throws Exception, Error {
	    ...
	    if(StringUtils.isEmpty(initialDate)){
	    	resultJSON += getAjaxTemplate(false, bundle.getString("queryCompetitivenessMatrix.initialDate.required"));
	    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryCompetitivenessMatrix.initialDate.required"));
	    }
	    
	    if(StringUtils.isEmpty(finalDate)){
	    	resultJSON += getAjaxTemplate(false, bundle.getString("queryCompetitivenessMatrix.finalDate.required"));
	    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryCompetitivenessMatrix.finalDate.required"));
	    }
	    
	    if(StringUtils.isEmpty(resultJSON)){
	    	resultJSON= queryCompetitivenessMatrixBean.getOffererCollection(initialDate, finalDate);
	    }else{
	...
	public String loadDestinationCountries(HttpServletRequest request,SessionManager managerDB) throws Exception, Error {
	    ...
	    if(StringUtils.isEmpty(offererCountry)){
	    	resultJSON += getAjaxTemplate(false, bundle.getString("queryCompetitivenessMatrix.offerer.required"));
	    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryCompetitivenessMatrix.offerer.required"));
	    }*/
	    
	    if(StringUtils.isEmpty(resultJSON)){
	    	resultJSON= queryCompetitivenessMatrixBean.getDestinationCollection(initialDate, finalDate/*, offererCountry*/);
	    }else{
	    	saveErrors(request, errors);
		    resultJSON = getErrors(request, false);
	    }
	...
	public String loadProducts(HttpServletRequest request,SessionManager managerDB) throws Exception, Error {
	    ...
	    if(StringUtils.isEmpty(destinationCountry)){
	    	resultJSON += getAjaxTemplate(false, bundle.getString("queryCompetitivenessMatrix.destination.required"));
	    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryCompetitivenessMatrix.destination.required"));
	    }
	    
	    if(StringUtils.isEmpty(resultJSON)){
	    	resultJSON= queryCompetitivenessMatrixBean.getProductsCollection(initialDate, finalDate, /*offererCountry,*/ destinationCountry);
	    }else{
	    	saveErrors(request, errors);
