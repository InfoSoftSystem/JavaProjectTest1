...
import org.promefrut.simefrut.struts.reports.beans.QueryProductionVolumeBean;
 ...
 *
 */
public class QueryProductionVolumeAction extends DispatchBaseAction {
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		QueryProductionVolumeBean queryProductionVolumeBean = new QueryProductionVolumeBean(managerDB, bundle);
		
		request.setAttribute("yearsCollection", queryProductionVolumeBean.getYearsCollection());
	...
	public ActionForward loadCountries(ActionMapping mapping, ActionForm form, HttpServletRequest request, 
		...
		try {
		    ...
		    if(StringUtils.isEmpty(initialDate)){
		    	resultJSON += getAjaxTemplate(false, bundle.getString("queryProductionVolume.initialDate.required"));
		    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryProductionVolume.initialDate.required"));
		    }
		    
		    if(StringUtils.isEmpty(finalDate)){
		    	resultJSON += getAjaxTemplate(false, bundle.getString("queryProductionVolume.finalDate.required"));
		    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryProductionVolume.finalDate.required"));
		    }
		    
		    if(StringUtils.isEmpty(resultJSON)){
		    	resultJSON= queryProductionVolumeBean.getCountriesCollection(initialDate, finalDate);
		    }else{
	...
	public ActionForward loadProducts(ActionMapping mapping, ActionForm form, HttpServletRequest request, 
		...
		try {
		    ...
		    if(StringUtils.isEmpty(countries)){
		    	resultJSON += getAjaxTemplate(false, bundle.getString("queryProductionVolume.countries.selected.required"));
		    	errors.add(BaseForm.GLOBAL_ERRORS, new ActionError("queryProductionVolume.countries.selected.required"));
		    }
		    
		    if(StringUtils.isEmpty(resultJSON)){
		    	resultJSON= queryProductionVolumeBean.getProductsCollection(initialDate, finalDate, countries);
		    }else{
		    	saveErrors(request, errors);
