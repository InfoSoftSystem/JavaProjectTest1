...
Ext.onReady(function() {
	...
	if(Ext.ux.statusbar.ValidationStatus){
		Ext.apply(Ext.ux.statusbar.ValidationStatus.prototype,{
			showText : 'Errores (clic para detalles)',
			hideText : 'Ocultar errores',
			submitText : 'Guardando...'
		});
	}
	
	if(Ext.ux.statusbar.StatusBar){
		Ext.apply(Ext.ux.statusbar.StatusBar.prototype,{
			busyText : 'Cargando...',
			text: 'Listo'
		});
	}
	...
	if(Ext.ux.grid){
		...
		if(Ext.ux.grid.filter.StringFilter){
			Ext.apply(Ext.ux.grid.filter.StringFilter.prototype, {
				emptyText: 'Ingrese Texto filtro'
			});
		}
		
		if(Ext.ux.grid.menu.ListMenu){
			Ext.apply(Ext.ux.grid.menu.ListMenu.prototype,{
				loadingText: 'Cargando...'
			});
		}
